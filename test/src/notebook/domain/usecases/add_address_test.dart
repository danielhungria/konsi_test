...
void main() {
  ...
  final tParams = AddAddressParams(address: tAddress);

  test('should add Cep to repository and return Right(null) when successful', () async {
    // arrange
    when(() => mockRepository.addAddress(any())).thenAnswer((_) async => const Right(null));
    // act
    final result = await usecase(tParams);
    // assert
    verify(() => mockRepository.addAddress(tAddress));
